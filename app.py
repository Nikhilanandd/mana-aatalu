...
def main():
    ...
    # Main header
    st.markdown(
        '<h1 class="main-header">మన ఆటలు (Mana Aatalu)</h1>', unsafe_allow_html=True
    )
    st.markdown(
        '<p class="telugu-text" style="text-align: center;">Our Traditional Childhood Games - Preserving Cultural Heritage</p>',
        unsafe_allow_html=True,
    )
    st.markdown("---")
...
def show_login_page(api_client):
    ...
    with st.form("login_form"):
        ...
        with col2:
            ...
            st.markdown(
                "Login to continue contributing to our traditional games collection."
            )
...
def show_signup_page(api_client):
    ...
    if st.session_state.signup_step == 1:
        ...
        with st.form("send_otp_form"):
            ...
            submitted_otp_send = st.form_submit_button(
        ...
        with st.form("verify_otp_form"):
            ...
            with col1:
                st.text_input(
                    "Phone Number",
                    value=st.session_state.signup_phone_number,
                    disabled=True,
                )
                otp_code = st.text_input(
                    "OTP Code", placeholder="Enter the 6-digit OTP"
                )
                name = st.text_input(
                    "Full Name",
                ...
                has_given_consent = st.checkbox(
                    ...
                    value=st.session_state.signup_consent,
                )

            with col2:
                st.markdown("### Join Our Community!")
                st.markdown(
                    "Help preserve traditional Indian childhood games for future generations."
                )
            ...
            submitted_otp_verify = st.form_submit_button(
                "Verify OTP & Create Account", use_container_width=True
            )
            resend_otp_button = st.form_submit_button(
                ...
                if not all([otp_code, name, email, password, confirm_password]):
                    ...
                    with st.spinner("Verifying OTP and creating account..."):
                        ...
                        if result:
                            ...
                            st.error(
                                "OTP verification failed or account creation error. Please check OTP and try again."
                            )

            if resend_otp_button:
                with st.spinner("Resending OTP..."):
                    result = api_client.resend_signup_otp(
                        st.session_state.signup_phone_number
                    )
                    if result:
...
def show_main_app(api_client):
    ...
    st.sidebar.markdown(
        f"**Welcome, {st.session_state.user_data.get('full_name', 'User')}!**"
    )
...
def show_home_page(api_client):
    ...
    *   **కనెక్ట్ అవ్వండి:** సాంప్రదాయ భారతీయ ఆటల స్ఫూర్తిని సజీవంగా ఉంచడానికి మాతో చేరండి!
    ...
    st.image(
        "https://code.swecha.org/nikhilanand/mana-aatalu/assets/mana.png",
        use_container_width=True,
    )  # Example image, replace with actual banner if available
...
def show_dashboard(api_client):
    ...
    with col1:
        ...
        if user_id:
            ...
            for media_type in media_types:
                response = api_client.get_user_contributions_by_media(
            ...
            if all_user_contributions:
                ...
                st.metric(
                    "Your Contributions",
                    "📝 0",
                    help="You haven't added any games yet.",
                )
        else:
            st.metric(
                "Your Contributions", "📝 N/A", help="Login to see your contributions"
            )
    ...
    with col2:
        ...
        st.metric(
            ...
            "🔒 N/A",
            help="Requires 'admin' or 'reviewer' role to view all contributors.",
        )
    ...
    )  # Changed title to reflect user's contributions
    ...
    with st.spinner("Loading your recent contributions..."):
        ...
        if user_id:
            ...
            if all_recent_contributions:
                ...
                st.success(
                    f"Found {len(all_recent_contributions)} of your recent contributions!"
                )
                for contrib in all_recent_contributions:
                    with st.container():
                        ...
                        with col1:
                            ...
                            st.caption(
                                f"Location: {location_display} | Added by: {st.session_state.user_data.get('full_name', 'You')}"
                            )
                        with col2:
...
def show_add_content_page(api_client):
    ...
    st.markdown(
        "Help preserve our cultural heritage by documenting traditional content!"
    )
    ...
    with st.form("add_content_form", clear_on_submit=True):
        ...
        content_text = st.text_area(
            "Detailed Content *",  # Changed label to reflect importance
        ...
        category_names = (
            [cat.get("name") for cat in categories_data if cat.get("name")]
            if categories_data
            else []
        )
        ...
        if not selected_category_id:
            st.error(
                "Selected category ID not found. Please try again or select a different category."
            )
        ...
        with col_lat:
            latitude_str = st.text_input(
                "Latitude", placeholder="e.g., 17.3850 (Optional)"
            )
            if latitude_str:
                try:
        ...
        with col_lon:
            longitude_str = st.text_input(
                "Longitude", placeholder="e.g., 78.4867 (Optional)"
            )
            if longitude_str:
                try:
        ...
        selected_release_rights_display = st.radio(
            "Release Rights *", list(release_rights_options.keys())
        )
        ...
        if submitted:
            ...
            mapped_release_rights = release_rights_options.get(
                selected_release_rights_display, "NA"
            )
            ...
            if not all(
                [
                    ...
                    selected_release_rights_display,
                ]
            ):  # All text fields are now required
                ...
                if (
                    len(combined_description) < 32
                ):  # Validate combined description length
                    st.error(
                        "The combined description (Title, Short Description, and Detailed Content) must be at least 32 characters long."
                    )
                else:
                    with st.spinner("Uploading your content contribution..."):
                        ...
                        if uploaded_files:
                            for uploaded_file in uploaded_files:
                                ...
                                file_upload_uuid = str(
                                    uuid.uuid4()
                                )  # Unique ID for THIS file's upload
                                ...
                                for i in range(total_chunks):
                                    ...
                                    if not chunk_result:
                                        st.warning(
                                            f"Failed to upload chunk {i + 1} for {uploaded_file.name}. Aborting upload."
                                        )
                                        current_file_upload_successful = False
                                        break
                                    last_chunk_result = chunk_result  # noqa: F841
                                ...
                                if current_file_upload_successful:
                                    ...
                                    st.error(
                                        f"File upload failed for {uploaded_file.name}."
                                    )
                        ...
                        if uploaded_files_info:
                            for file_info in uploaded_files_info:
                                ...
                                if "audio" in media_type_full:
                                    ...
                                    "image" in media_type_full
                                    or "pdf" in media_type_full
                                ):  # PDF is treated as image by backend
                                    media_type = "image"
                                else:
                                ...
                                finalize_result = api_client.finalize_record_upload(
                                    ...
                                    total_chunks=file_info[
                                        "total_chunks"
                                    ],  # Use the stored total_chunks for this file
                                ...
                                if not finalize_result:
                                    st.warning(
                                        f"Failed to finalize record for {file_info['name']}."
                                    )
                                    uploaded_files_count -= 1

                            if uploaded_files_count > 0:
                                st.success(
                                    f"🎉 {uploaded_files_count} files uploaded and content finalized successfully! Thank you for contributing to our cultural heritage!"
                                )
                                st.balloons()
                                ...
                                st.markdown(
                                    f"**Files Uploaded:** {uploaded_files_count} files"
                                )
                                st.info(
                                    "Your contribution is now part of our community collection and visible to all users!"
                                )
                            else:
                                st.error("Failed to upload content. Please try again.")
                        else:
                            st.error(
                                "No files were selected for upload or all uploads failed. Please try again."
                            )
            ...
            with st.spinner("Loading user profile..."):
                ...
                if user:
                    ...
                    with col1:
                        ...
                        st.markdown(
                            f"**Date of Birth:** {user.get('date_of_birth', 'N/A')}"
                        )
                        st.markdown(f"**Place:** {user.get('place', 'N/A')}")
                        st.markdown(
                            f"**Member Since:** {user.get('created_at', 'Recently')}"
                        )
                        st.markdown(
                            f"**Last Login:** {user.get('last_login_at', 'Recently')}"
                        )

                    with col2:
                        st.info(
                            "Contribution statistics are not available with the current API endpoint."
                        )
                else:
                ...
                if st.form_submit_button("Update Password"):
                    if new_password != confirm_new_password:
                        ...
                        with st.spinner("Changing password..."):
                            change_result = api_client.change_password(
                                st.session_state.access_token, new_password
                            )  # Assuming current password is not needed for this API, or handled by token
                            if change_result:
        ...
        with st.expander("📧 Update Profile"):
            with st.form("update_profile_form"):
                updated_name = st.text_input(
                    "Name", value=user.get("name", "")
                )  # Use 'name' from new schema
                ...
                if st.form_submit_button("Update Profile"):
                    if not validate_email(updated_email):
                        ...
                        with st.spinner("Updating profile..."):
                            update_result = api_client.update_user(
                                user.get("id"), user_update_data
                            )  # Assuming user ID is available in session_state.user_data
                            if update_result:
