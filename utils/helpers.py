...
def filter_contributions(
    contributions: List[Dict], filters: Dict[str, Any]
) -> List[Dict]:
    ...
    if filters.get("search_term"):
        ...
        filtered = [
            ...
            for c in filtered
            if (
                search_lower in c.get("metadata", {}).get("game_name", "").lower()
                or search_lower in c.get("metadata", {}).get("description", "").lower()
                or search_lower in c.get("metadata", {}).get("rules", "").lower()
            )
        ]
...
def generate_game_stats(contributions: List[Dict]) -> Dict[str, Any]:
    ...
    stats = {
        ...
        "regions": len(
            set(c.get("metadata", {}).get("region", "Unknown") for c in contributions)
        ),
        "categories": len(
            set(c.get("metadata", {}).get("category", "Unknown") for c in contributions)
        ),
    ...
    categories = [
        c.get("metadata", {}).get("category", "Unknown") for c in contributions
    ]
